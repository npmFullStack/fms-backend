...
// Get all containers for a line (for booking - only returned containers)
...
// Get ALL containers for a line (for management table - both returned and in-use)
export const getAllLineContainers = async (req, res) => {
  try {
    const { shippingLineId } = req.params;
    const containers = await getAllContainersByLine(shippingLineId);
    res.json(containers);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
