...
import { truckingCompanySchema } from "../schemas/truckingCompanySchema.js";
...
export const addTruckingCompany = async (req, res) => {
  ...
  try {
    ...
    // Parse service routes from request body
    const serviceRoutes = req.body.serviceRoutes 
      ? JSON.parse(req.body.serviceRoutes)
      : [];
    ...
    // Validate data
    truckingCompanySchema.parse(truckingCompanyData);
    ...
    await createTruckingCompany(
      ...
      truckingCompanyData.contactEmail,
      truckingCompanyData.contactPhone,
      truckingCompanyData.serviceRoutes
    );
...
export const editTruckingCompany = async (req, res) => {
  ...
  try {
    ...
    const serviceRoutes = req.body.serviceRoutes 
      ? JSON.parse(req.body.serviceRoutes)
      : currentTruckingCompany.rows[0].service_routes || [];

    const truckingCompanyData = {
      name: req.body.name || currentTruckingCompany.rows[0].name,
      logoUrl: logoUrl,
      contactEmail: req.body.contactEmail || currentTruckingCompany.rows[0].contact_email || null,
      contactPhone: req.body.contactPhone || currentTruckingCompany.rows[0].contact_phone || null,
      serviceRoutes: serviceRoutes,
    };
...
export const toggleStatus = async (req, res) => {
  try {
    ...
    res.json({
      message: `Trucking company ${result.rows[0].is_active ? "activated" : "deactivated"}`,
